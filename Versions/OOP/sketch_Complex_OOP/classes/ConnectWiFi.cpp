...
ConnectWiFi : ConnectWiFi(server.ip ip, server.net net, server.gw gw, bool accesspoint, int stationCount) {
  _ip = ip;
  _net = net;
  _gw = gw;
...
void init() {
  ...
  _accesspoint = false _stationCount = sizeof(stationList) / sizeof(stationList[0]);
